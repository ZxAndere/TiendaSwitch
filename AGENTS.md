...
- **Admin audit + import**: `logAudit()` (server.js:342) logs admin mutations to Mongo + `data/audit-log.json` (cap 2000); view via `GET /api/admin/audit?limit=`. Bulk game import: `POST /api/admin/juegos/import/preview` (dry-run) + `/commit` (revalidates all rows, never trusts preview); body `{rows:[...]}` or `{csv:"..."}` (server parses CSV via `csv-parse`, client never does); max 200 rows; unknown keys rejected — `cuentas`/`sold*`/`siguienteVarianteIndex`/`deletedAt` are NOT importable; duplicates (id or normalized titulo) skipped; `createdAt` is whitelisted (round-trips with the admin CSV export). Global `express.json` limit is 1mb (server.js:111) — do not lower it below what imports need.
- **Analytics + privacidad**: `POST /api/analytics/event` is PUBLIC (rate-limited 200/15min, `analyticsLimiter`) — anonymous telemetry, types `game_view`/`add_to_cart`/`checkout_start`/`purchase`/`favorite_add`/`favorite_remove`/`search`, never stores PII, never 401s (bad tokens ignored); storefront helper `trackAnalytics()` (app.js:149) checks the privacy gate first. Aggregates: `GET /api/admin/analytics?days=` (verifyAdmin). Opt-out: user field `trackingOptOut` via `POST /api/user/tracking` (verified in saveUsers/req.user/startup sync — keep it there), guests use `localStorage['zonaswitch_no_track']`. Games carry `createdAt` (set on create/import; update can't overwrite). `POST /api/admin/juegos/delete` with `{permanent:true}` hard-deletes (memory + Mongo + JSON + audit `juegos.delete_permanent`); without the flag it stays a soft delete.
