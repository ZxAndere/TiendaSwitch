...
- **Admin audit + import**: `logAudit()` (server.js:342) logs admin mutations to Mongo + `data/audit-log.json` (cap 2000); view via `GET /api/admin/audit?limit=`. Bulk game import: `POST /api/admin/juegos/import/preview` (dry-run) + `/commit` (revalidates all rows, never trusts preview); body `{rows:[...]}` or `{csv:"..."}` (server parses CSV via `csv-parse`, client never does); max 200 rows; unknown keys rejected — `cuentas`/`sold*`/`siguienteVarianteIndex`/`deletedAt` are NOT importable; duplicates (id or normalized titulo) skipped. Global `express.json` limit is 1mb (server.js:111) — do not lower it below what imports need.
